...
import threading
lock_for_importer = threading.Lock()

# --------------------------- 17/03/2020-----------------------------------
# slm5cob/wia6cob : In the method def __setitem__()of class importcache, a lock is
#           acquired before the command 
#              self._items[self._keys.index(key)] = item
#           and then released, so that no two threads can execute it at the
#           same time.
# -------------------------------------------------------------------------
...
class ImportCache(object):
    ...
    def __setitem__(self, key, item):
        ...
        if key not in self._keys:
            ...
            lock_for_importer.acquire()
            self._items[self._keys.index(key)] = item
            lock_for_importer.release()
