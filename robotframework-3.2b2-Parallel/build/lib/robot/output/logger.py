...
# --------------------------- 17/03/2016-----------------------------------
# slm5cob/wia6cob : instead of creating a single instance LOGGER,a new api
#           new_logger_for_thread() is called, which returns a unique
#           instance for separate threads.
# -------------------------------------------------------------------------
...
def new_logger_for_thread():
    if threading.current_thread() in thread_Logger_obj_dict.keys():
        return thread_Logger_obj_dict[threading.current_thread()]
    else:
        Logger_obj = Logger()
        thread_Logger_obj_dict[threading.current_thread()] = Logger_obj
        return thread_Logger_obj_dict[threading.current_thread()]

LOGGER = new_logger_for_thread()
