...
# --------------------------- 27/10/2016-----------------------------------
# slm5cob : instead of creating a single instance for RobotFramework class,
#           in run() api, unique instances are created for each thread,
#           and are stored in global dictionary thread_robot_dict.
#
#           Also, all the class variables in RobotFramework class are made
#           as instance variables, so each instance of the class has it's
#           own 'logger', 'settings' and 'suite'.
# -------------------------------------------------------------------------
...
from robot.output.logger import new_logger_for_thread
from robot.reporting import ResultWriter
from robot.running import TestSuiteBuilder
from robot.utils import Application, unic
import threading
global thread_robot_dict
thread_robot_dict = {}
from robot.output.logger import *
from robot.running.context import *
lock_for_run = threading.Lock()
...
class RobotFramework(Application):
    ...
    def main(self, datasources, **options):
        ...
        self.suite = TestSuiteBuilder(self.settings['SuiteNames'],
...
def run(*datasources, **options):
    global thread_robot_dict
    global thread_Logger_obj_dict
    global thread_exec_context_dict
    ...
    #count = len(robot_obj_list)
    #new_obj = RobotFramework()
    #robot_obj_list.append(new_obj)
    #lock_for_run.release()
    #return robot_obj_list[count].execute(*datasources, **options)
    robot_exec_obj = RobotFramework()
    thread_robot_dict[threading.current_thread()] = robot_exec_obj
    temp_obj = thread_robot_dict[threading.current_thread()].execute(*datasources, **options)
    del thread_robot_dict[threading.current_thread()]
    del thread_Logger_obj_dict[threading.current_thread()]
    del thread_exec_context_dict[threading.current_thread()]
    return temp_obj
