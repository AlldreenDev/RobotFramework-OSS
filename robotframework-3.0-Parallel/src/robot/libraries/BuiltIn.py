...
from robot.running.context import unique_executioncontexts_obj_for_thread
...
class _BuiltInBase(object):
    ...
    def _get_context(self, top=False):
        ctx = unique_executioncontexts_obj_for_thread().current if not top else unique_executioncontexts_obj_for_thread().top
        if ctx is None:
