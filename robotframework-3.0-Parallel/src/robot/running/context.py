...
# --------------------------- 27/10/2016-----------------------------------
# slm5cob : instead of creating a single instance unique_executioncontexts_obj_for_thread for
#           class ExecutionContexts, a new api unique_executioncontexts_obj_for_thread() is
#           called, which returns a unique instance for separate threads.
# -------------------------------------------------------------------------
...
# Instead of creating an instance, this api will return new instance for each thread
def unique_executioncontexts_obj_for_thread():
    if threading.current_thread() in thread_exec_context_dict.keys():
        return thread_exec_context_dict[threading.current_thread()]
    else:
        EXECUTION_CONTEXTS_obj = ExecutionContexts()
        thread_exec_context_dict[threading.current_thread()] = EXECUTION_CONTEXTS_obj
        return thread_exec_context_dict[threading.current_thread()]
...
#unique_executioncontexts_obj_for_thread = ExecutionContexts()
...
class _ExecutionContext(object):
    ...
    def end_suite(self, suite):
        ...
        unique_executioncontexts_obj_for_thread().end_suite()
